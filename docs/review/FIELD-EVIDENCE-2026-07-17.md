...
# Addendum — 2026-07-22 (closing): the two readers and the lineage boundary (operator, feeds Slice G)

The operator articulated the reader loop's design constraint: the capsule's reader should be an agent, not a human — but an agent inside the project is "grading itself... even if the witness isn't itself." Sharpened into the standing distinction:

- **Continuation reader** (next builder in the same project): inside the lineage by design, and NOT self-grading — contamination flows through narrative, and the receipt is the non-narrative channel; the reader verifies a hash-chained record the prior agent could not edit. This is the verified continuation (Part F, roadmap Step 6).
- **Audit reader** (does the record hold up?): must be OUTSIDE the project lineage — fresh instance, zero project context, ideally another platform — or it is the 24-subagent trap again, and its credit is worthless as evidence. The operator's own fresh-agent isolation leg (B09) is the hand-built prototype; the Slice B blind-reader gate is this loop run manually.

Delivery requires no human clipboard: the packet is local files any same-machine agent can read (a fresh thread, a headless instance with an empty home). The missing piece is the one gesture — a standing reader brief (verify chain, read packet, report only what the record supports; no other context) plus a launcher that spawns a clean instance pointed at a run folder. Mostly skill/script, not engine code; productizes as Slice G to this constraint. Lyhna never grades — the reader does, as attributed testimony; the reading itself is witnessable through the read tool.

---

# Addendum — 2026-07-26: the published bundle, installed cold (session-run, not an operator field gate)

**What this is, and what it is not.** A session took the **published v0.1.30 release bundle** — the artifact a stranger would download — and ran it cold: unzipped to a fresh location with no repository, no git above it, no `npm install`, and an isolated data root. This is a lab proof of the shipped artifact by a session, on synthetic runs. It is **not** an operator field run and **does not** satisfy either owed blind-reader gate: Slice B still needs a sealed packet from Adam's real work read cold by blind readers, and CZ-14 still needs a cold open packet. Both remain owed and Adam-paced. Recorded here because the shipped artifact had never been exercised as a stranger receives it, and because of what it found.

**What held.** Layout matched the marketplace manifest (`.agents/plugins/marketplace.json` → `./plugins/lyhna`). Version agreement across all three manifests at `0.1.30`. Every advertised command actually ran — `validate:plugin`, `key`, `verify:lineage` — no phantom commands. A two-window lineage chain driven entirely from bundle code sealed both windows. `verify:lineage` on the clean pair returned `LINKED`, ten of ten checks, signature verifying, exit 0. Objective and claim-text retention from 0.1.29/0.1.30 were present in the shipped `HANDOFF.md`. A tampered prior ledger produced `NOT LINKED`, `FAIL prior_chain_valid`, exit 1.

**What it found — and it was the report, not the verdict.** The clean run printed ten check rows; the tampered run printed nine. `prior_continuation_refolds` had not failed, it had disappeared, with nothing anywhere stating it had not been evaluated. The verdict was never wrong — a broken chain fails on its own — but a reader diffing two reports saw a row vanish rather than a reason, and "we could not check this" read as "this did not apply." That is CZ-15, fixed and shipped in v0.1.31 (`539e67f`); the build record is in `BUILD-PLAN-2026-07-16.md`.

**The lesson worth keeping, since it is the product's own thesis turned inward.** The defect was not reachable from the repository. Tests were green, the validator was green, CI was green, and the released artifact was internally correct. It surfaced only when someone used the shipped thing the way a stranger would and **compared two reports side by side** — the exact reading motion the receipt exists to support. A green suite proved the code did what it was written to do; it could not tell anyone the report was quietly incomplete. This is the same shape as the 2026-07-22 process lesson one layer up: the ledger, not the testimony. Here, the artifact, not the suite.

**Standing correction, small and cheap:** exercise the published bundle cold after each release, and diff a clean report against a deliberately broken one. Both defects this found were visible in the diff and invisible in the pass/fail.
