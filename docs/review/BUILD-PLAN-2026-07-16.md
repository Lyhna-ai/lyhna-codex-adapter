...
### Slice B2 — The Run Closes Itself; the Packet Arrives *(operator-directed 2026-07-18; builds immediately after the Slice B gate)*
The CZ-13 escalation, second occurrence. The operator's directive (2026-07-18, verbatim substance): a working agent in another project must **never** be asked to focus on the witness closeout — it confuses that agent's own closeout (observed twice, told to the last agent directly); and the operator cannot keep hunting for witness files, zipping data roots, and running extraction errands — the sealed record needs an easy place to arrive (the lyhna-witness site's agent inbox is the reference picture). Two requirements:

1. **Zero witness instructions to working agents.** No closeout-doc asks, no refresh-before-close ritual. Close-time snapshot refresh is mechanized inside the adapter (observing a moved head is an observation, not a judgment — the engine does it itself when close is requested, so superseded evaluations resolve to `STALE` by mechanism, not by agent choreography). Close intent arrives as at most one zero-narration gesture — or from the operator directly, outside the working agent's task stream; mechanism decided at spec time.
2. **The sealed packet is one gesture away.** Auto-export on seal to a human-named packets directory with `RECEIPT.md` as the named front page (local, zero new dependencies, deterministic). Delivery beyond the machine — the witness-site agent inbox for humans and outside agents — is scoped at spec time; the local one-gesture export ships first. (Absorbs the one-gesture packet work item recorded in `FIELD-EVIDENCE-2026-07-17.md` §11, previously parked as Slice C scope; the abandoned temp-file cleanup item rides along if it fits.)

**Field gate:** a real run seals with no witness instruction to the working agent beyond (at most) a single close gesture, and Adam reaches the sealed packet in one gesture — no hunting, no zipping, no agent errands.
