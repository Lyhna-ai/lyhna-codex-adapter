...
> **Status (2026-07-26): ACTIVE — shipping from `main`.** `main` is at **v0.1.30** (`d06ad88`), published as an installable release bundle on 2026-07-25 by the release workflow and since **proven cold** — installed from the zip with no repo and no `npm install`, driven through a two-window lineage chain, tamper-checked. **Thesis v2 was ratified on 2026-07-25** and is now canonical, with two recorded amendments. Slice A remains closed and field-proven. Slice B and CZ-14 are code-final; **both blind-reader field gates are still owed**, and no gate run has been recorded since 2026-07-23. The full stack (PRs #1–#3) merged 2026-07-23; PRs #6–#9 have merged since. **PR #10 is the only open PR** — draft, v0.1.31, `verify:lineage` reporting. PRs #4 and #5 were closed on 2026-07-26 as superseded, their documents landed here first.
...
| #10 | 0.1.31 | *Open, draft.* `verify:lineage` emits a fixed check sequence with `NOT RUN` plus a reason, instead of silently dropping checks it could not evaluate. |
...
- **Build records for PRs #6–#9 are not in `BUILD-PLAN-2026-07-16.md`.** The plan's records stop at CZ-14 / 2026-07-23; four merged builds and a published release are unrecorded there. The PR bodies carry the reasoning in the meantime.
- **`FIELD-EVIDENCE-2026-07-17.md` has no entry after 2026-07-22.** Nothing from the v0.1.28–v0.1.30 window, including the cold-install proof of the published bundle, has been appended to the field record.

Timeline context: the F1–F6 findings in the dossier were made against PR #1 head `f61de1e`; all six were closed by the builder at head `013c70f`. The customer-zero review was made against a live run of the installed adapter plus code inspection at `013c70f`, and defines CZ-1 through CZ-7. Slice A's gate passed twice in the field on 2026-07-17. The full stack merged to `main` on 2026-07-23 at v0.1.27; the continuation lane, Verified Context, and capsule signing landed across 2026-07-24/25, and v0.1.30 was published as a release on 2026-07-25.
