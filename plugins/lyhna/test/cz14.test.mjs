...
test('a plain checkpoint writes an anchor and receipts, and the face reads OPEN', { concurrency: false }, (t) => {
  ...
  assert.equal(anchorEvent.payload.receipt_renderer, '0.1.28');
  ...
  assert.equal(anchor.receipt_renderer, '0.1.28');
  ...
  assert.equal(events.at(-2).payload.receipt_renderer, '0.1.28');
...
test('a re-deferred close after intervening events stays fully verifiable and forge-proof', { concurrency: false }, (t) => {
  ...
  assert.equal(anchors.at(-1).payload.receipt_renderer, '0.1.28');
...
test('a redelivered Stop whose checkpoint is still the ledger tip does not seal', { concurrency: false }, (t) => {
  ...
  appendEvent(run.id, {
    ...
    origin: 'runtime_hook',
    payload: { status: 'OPEN', receipt_renderer: '0.1.28' },
...
test('a redelivered Stop finalizes a seal whose run_sealed event is already in the ledger', { concurrency: false }, (t) => {
  ...
  appendEvent(run.id, { type: 'turn_checkpoint', origin: 'runtime_hook', payload: { status: 'OPEN', receipt_renderer: '0.1.28' }, idempotencyKey: 'checkpoint:stop-1' });
  appendEvent(run.id, { type: 'run_sealed', origin: 'runtime_hook', payload: { status: 'SEALED', receipt_renderer: '0.1.28' }, idempotencyKey: `seal:${run.id}` });
...
test('a ledger with a second run_sealed after a post-seal write fails closed', { concurrency: false }, (t) => {
  ...
  rawAppendEvent(directory, { type: 'run_sealed', origin: 'runtime_hook', payload: { status: 'SEALED', receipt_renderer: '0.1.28' }, idempotencyKey: 'seal:duplicate' });
...
test('a replayed Stop recovers a lagging state prefix before completing the packet', { concurrency: false }, (t) => {
  ...
  rawAppendEvent(directory, { type: 'turn_checkpoint', origin: 'runtime_hook', payload: { status: 'OPEN', receipt_renderer: '0.1.28' }, idempotencyKey: 'checkpoint:stop-1' });
