...
test('same-turn host continuations advance while an exact completed-response replay remains idempotent', { concurrency: false }, (t) => {
  const data = isolatedData(t);
  const sessionId = 'compiler-same-turn-host-continuations';
  const parent = mintSession({ sessionId, cwd: process.cwd() });
  const run = beginRun(parent, { mode: 'full', objective: 'Close unsupported after bounded same-turn continuations.' });
  declareClaimContract(parent, contract());
  requestClose(parent, 'Close only through the declared claim gate.');
  const env = { ...process.env, LYHNA_CODEX_DATA: data };
  const stop = (message, stopHookActive) => ({
    hook_event_name: 'Stop',
    session_id: sessionId,
    turn_id: 'same-host-turn',
    stop_hook_active: stopHookActive,
    last_assistant_message: message
  });

  const firstInput = stop('First bounded completion response.', false);
  const first = runHook(firstInput, env);
  assert.equal(first.decision, 'block');
  assert.equal(getRunForTesting(run.id).events.filter((event) => event.type === 'closeout_attempted').length, 1);

  const replayedFirst = runHook(firstInput, env);
  assert.equal(replayedFirst.decision, 'block');
  assert.equal(getRunForTesting(run.id).events.filter((event) => event.type === 'closeout_attempted').length, 1);

  const secondInput = stop('Second bounded completion response after the hook continuation.', true);
  const second = runHook(secondInput, env);
  assert.equal(second.decision, 'block');
  assert.equal(getRunForTesting(run.id).events.filter((event) => event.type === 'closeout_attempted').length, 2);

  const replayedSecond = runHook(secondInput, env);
  assert.equal(replayedSecond.decision, 'block');
  assert.equal(getRunForTesting(run.id).events.filter((event) => event.type === 'closeout_attempted').length, 2);

  const third = runHook(stop('Third bounded completion response after the second continuation.', true), env);
  assert.deepEqual(third, {});
  const final = getRunForTesting(run.id);
  assert.equal(final.state.terminal_status, 'CLOSED_UNSUPPORTED');
  assert.equal(final.events.filter((event) => event.type === 'closeout_attempted').length, 3);
  assert.deepEqual(final.events.filter((event) => event.type === 'closeout_attempted').map((event) => event.payload.ordinal), [1, 2, 3]);
});

test('proof-mode same-turn Stop identity preserves liveness without persisting assistant prose', { concurrency: false }, (t) => {
  const data = isolatedData(t);
  const sessionId = 'compiler-proof-same-turn-host-continuations';
  const parent = mintSession({ sessionId, cwd: process.cwd() });
  const run = beginRun(parent, {
    mode: 'full',
    objective: 'Close unsupported after proof-mode same-turn continuations.',
    privacyMode: 'proof'
  });
  declareClaimContract(parent, contract());
  requestClose(parent, 'Close only through the declared claim gate.');
  const env = { ...process.env, LYHNA_CODEX_DATA: data };
  const messages = [
    'RAW_PROOF_STOP_COMPLETION_ONE',
    'RAW_PROOF_STOP_COMPLETION_TWO',
    'RAW_PROOF_STOP_COMPLETION_THREE'
  ];
  const stop = (message, stopHookActive) => ({
    hook_event_name: 'Stop',
    session_id: sessionId,
    turn_id: 'same-proof-host-turn',
    stop_hook_active: stopHookActive,
    last_assistant_message: message
  });

  assert.equal(runHook(stop(messages[0], false), env).decision, 'block');
  const secondInput = stop(messages[1], true);
  assert.equal(runHook(secondInput, env).decision, 'block');
  assert.equal(runHook(secondInput, env).decision, 'block', 'an exact completed response replay remains idempotent');
  assert.deepEqual(runHook(stop(messages[2], true), env), {});

  const final = getRunForTesting(run.id);
  assert.equal(final.state.terminal_status, 'CLOSED_UNSUPPORTED');
  assert.equal(final.events.filter((event) => event.type === 'closeout_attempted').length, 3);
  const packetBytes = readTree(final.directory).join('\n');
  for (const message of messages) assert.equal(packetBytes.includes(message), false);
});
