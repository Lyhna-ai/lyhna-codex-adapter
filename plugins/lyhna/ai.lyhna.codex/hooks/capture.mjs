...
try {
  ...
  if ((hookInput.hook_event_name || hookInput.event_name) === 'Stop' && error.lyhnaClaimBoundary === true) {
    const reason = `BLOCKED_TRANSPORT: Lyhna could not persist or verify the closeout boundary (${code}). The run remains unsupported; repair transport or ledger integrity before closing.`;
