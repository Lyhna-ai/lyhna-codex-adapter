...
- A sealed capsule is signed with a local Ed25519 key, and the public key is embedded in the capsule (trust-on-first-use). The key lives beside the packets under the data root, written owner-only, minted on first use. `key_id` is parameterized so a per-job or per-project key is configuration rather than redesign; v0 uses one default key.
