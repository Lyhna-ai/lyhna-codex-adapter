...
- **Fold dispatch reads the hash chain, never the capsule.** The generation is derived from the `receipt_renderer` committed in the `run_sealed` or latest `checkpoint_anchor` event. The capsule declares `continuation_fold_version` for a reader, but verification must never dispatch on it: the capsule is unanchored, so trusting that field would let a forged packet select the reducer under which it verifies — a cache selecting a weaker verification path. A renderer version the checker cannot place yields `NOT_RUN` and fails safe; it is never folded with current rules.
