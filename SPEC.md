...
- Every lineage report carries every check, in a fixed order. A check whose input was destroyed by an earlier failure is reported `NOT RUN` with the reason it could not be evaluated; it is never omitted. Dropping the row would let "this could not be checked" read as "this did not apply" — the collapse `UNRESOLVED_EVIDENCE` exists to prevent one layer down — and would let a row silently disappear between two reports a reader is comparing. A `NOT RUN` check is not a pass: the verdict requires every check to have actually passed, so an unknown fails safe even if some future path reaches it without an accompanying failure.
