...
- **An agent-authored event is never evidence.** A reference that resolves to an `agent_reported` event — another builder claim — does not support anything, and a claim whose references resolve only to such events is `UNSUPPORTED`. It is not `UNRESOLVED_EVIDENCE`: the reference resolved, so saying otherwise would be its own overclaim. The ledger witnesses that a claim was *made*; it never witnesses that it was *true*. Without this rule two identical claims, the second citing the first, are enough to launder narration into witnessed evidence and from there into `settled`.
